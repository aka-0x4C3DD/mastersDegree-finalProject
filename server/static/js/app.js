...
function processResponse(data, originalQuery) {
    ...
    if (data.web_results && data.web_results.length > 0) {
        responseContent += `<div class="web-results">
            <p class="fw-bold">Information from trusted medical sources:</p>`;
        
        data.web_results.forEach(result => {
            responseContent += `
                <div class="web-result">
                    ${result.title ? `<div class="web-result-title">${result.title}</div>` : ''}
                    <div class="web-result-source">Source: <a href="${result.source}" target="_blank">${result.source} <i class="fas fa-external-link-alt fa-xs"></i></a></div>
                    <div class="web-result-content">${highlightMedicalTerms(result.content)}</div>
                </div>
            `;
        });
...
function formatAIResponse(data) {
    ...
    if (typeof data === 'string') {
        // If data is a string, just highlight medical terms
        html = highlightMedicalTerms(data);
    } else {
        // If data has a response property
        if (data.response) {
            html += `<p>${highlightMedicalTerms(data.response)}</p>`;
        }
        
        // If data has web results
        if (data.web_results && data.web_results.length > 0) {
        ...
        // If there's file information
        if (data.file_type) {
            html += `<div class="file-info">
                <div class="file-info-header">
                    <div class="file-info-icon"><i class="fas fa-file-medical"></i></div>
                    <div class="file-info-name">${data.file_name || 'File Analysis'}</div>
                </div>
                <div class="file-info-details">
                    <strong>File Type:</strong> ${data.file_type.toUpperCase()}
                    ${data.rows && data.columns ? `<br><strong>Data:</strong> ${data.rows} rows, ${data.columns} columns` : ''}
                </div>
            </div>`;
        }
    }
